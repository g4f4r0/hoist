...
function generateClaudeSkill(): string {
  return [
    ...
    "## When NOT to Use",
    "",
    "- **Interactive setup** (`hoist provider add`, `hoist provider update`) — require human API key input. Tell the user to run these.",
    ...
    "    +-- YES -> Run: hoist init --json --yes (auto-configures from env vars)",
    "    +-- NO  -> Tell user to run: hoist init (requires human API key input)",
    ...
    "",
    "**NEVER run these without `--json`/`--yes`.** They prompt for API keys interactively:",
    "",
    "| Command | User action |",
    "|---------|------------|",
    "| `hoist init` (without `--json`) | Enter provider type + API key |",
    ...
    "",
    "**Exception:** `hoist init --json --yes` is safe for agents — it reads provider keys from environment variables (`HOIST_HETZNER_API_KEY`, `HOIST_VULTR_API_KEY`, `HOIST_DIGITALOCEAN_API_KEY`) instead of prompting.",
    "",
    ...
    "| Error | Solution |",
    "|-------|----------|",
    '| "Run hoist init first" | If env vars are set, run `hoist init --json --yes`. Otherwise tell user to run `hoist init`. |',
...
function generateCommandsReference(): string {
  return [
    ...
    "",
    "```bash",
    "hoist init                                # Interactive setup (human only)",
    "hoist init --json --yes                   # Non-interactive, reads HOIST_*_API_KEY env vars",
    "```",
    "",
    "Environment variables for non-interactive init:",
    "- `HOIST_HETZNER_API_KEY` — Hetzner Cloud API key",
    "- `HOIST_VULTR_API_KEY` — Vultr API key",
    "- `HOIST_DIGITALOCEAN_API_KEY` — DigitalOcean API key",
    "",
    "## Providers",
    "",
    "```bash",
