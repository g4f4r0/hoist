...
function generateInstructions(config?: ProjectConfig): string {
  const lines: string[] = [
    ...
    "| `hoist db create` | Create a database |",
    "| `hoist db list` | List databases |",
    "| `hoist db info <name>` | Get database connection details |",
    "| `hoist db destroy <name>` | Destroy a database |",
    "| `hoist db stop/start/restart <name>` | Control a database |",
...
function generateClaudeSkill(config?: ProjectConfig): string {
  const lines: string[] = [
    ...
    "",
    "# Databases",
    "hoist db create --name <n> --type <type> --server <s> --json --yes",
    "hoist db list --json",
    "hoist db info <name> --json",
    "hoist db destroy <name> --json --yes",
    "hoist db stop/start/restart <name> --json",
    "",
    "# Domains",
